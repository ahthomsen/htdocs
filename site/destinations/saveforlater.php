...
//checks to see if user was browsing using the destiny function
...
if(isset($_SESSION['userid']))
{
	...
	$destid = $_GET['dest'];
		...
		try 
		{
			$results = $db->prepare("SELECT 1 from save_for_later WHERE USERID=? AND DESTID=?");
		...
		try {
			$update = $db->prepare('INSERT INTO save_for_later (DESTID, USERID) VALUES (?, ?);');
